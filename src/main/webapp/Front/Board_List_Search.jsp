...
<% 
	...
	int RowCount = board.RowCount_Target(name, board_type);
	
	if(RowCount==0)
		out.println("<script>alert('입력하신 "+name+"와(과) 일치하는 "+board_category+"가 없습니다.');history.go(-1);</script>");
	...
	ArrayList<GetSet> Page = board.Page_Title(name, board_type, StartNum);
...
<body>
	...
	<article>
      <div class="container">
		...
		<a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Board_List_Search.jsp&name=<%=name %>&board_type=<%=board_type%>">처음으로</a>
		<a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Board_List_Search.jsp&name=<%=name %>&board_type=<%=board_type%>&pageNum=<%=Minus10_Page %>">-10</a>
		<a class="mb-2 mr-2 btn-transition btn btn-outline-info"
		<%if(CurrentPage>1) {%>
			href="mainPage.jsp?center=Board_List_Search.jsp&name=<%=name %>&board_type=<%=board_type%>&pageNum=<%=CurrentPage-1 %>"
		<%}else{ %>
			href="#"
		<%} %>
		>이전</a>
		<%for(int i=startPage;i<=endPage;i++){ %>
			<a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Board_List_Search.jsp&name=<%=name %>&board_type=<%=board_type%>&pageNum=<%=i %>"><%=i %></a>
		<%} %>
		<a class="mb-2 mr-2 btn-transition btn btn-outline-info"
		...
		<%} %>
		>다음</a>
		<a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Board_List_Search.jsp&name=<%=name %>&board_type=<%=board_type%>&pageNum=<%=Plus10_Page %>">+10</a>	
		<a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Board_List_Search.jsp&name=<%=name %>&board_type=<%=board_type%>&pageNum=<%=MaxPage %>">끝으로</a>	
		</div>
		<div class="col text-center">
