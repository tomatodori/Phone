...
<%
   ...
   //SELECT COUNT(*) FROM ITEM WHERE ITEM_GROUP='"+group+"'";
...
<body>
  <form action="mainPage.jsp">
   ...
   <table>
      <tr>
         <td>
            <input type="text" name="name" placeholder="상품의 이름을 입력해주세요.">
         </td>
         <td>
  ...
  <section class="py-5">
   ...
   <a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="../Front/mainPage.jsp?center=Item_List_Group.jsp&group=<%=group%>&pageNum=1">처음으로</a>
   <a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="../Front/mainPage.jsp?center=Item_List_Group.jsp&group=<%=group%>&pageNum=<%=Minus10_Page%>">-10</a>
   
   <a class="mb-2 mr-2 btn-transition btn btn-outline-info" 
    <%if (CurrentPage > 1) {%>
       href="mainPage.jsp?center=Item_List_Group.jsp&group=<%=group%>&pageNum=<%=CurrentPage - 1%>"
    <%} else {%> 
       href="#" 
    <%}%>
    >이전</a>
   ...
   <%for (int i = startPage; i <= endPage; i++) {%>
         <a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Item_List_Group.jsp&group=<%=group%>&pageNum=<%=i%>"><%=i%></a>
   <%}%>
   ...
   <%}%>
   >다음</a>
   <a class="mb-2 mr-2 btn-transition btn btn-outline-info" href="mainPage.jsp?center=Item_List_Group.jsp&group=<%=group%>&pageNum=<%=Plus10_Page%>">+10</a>
